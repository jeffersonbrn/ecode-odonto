 ...
 */
class DevolvidosList extends TPage
{
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_DevolvidosList');
        $this->form->setFormTitle('Lista de Devolvidos');
        ...
        $name->setValue( TSession::getValue( 'Nome_DevolvidosList' ) );
        ...
        $col_curso = new TDataGridColumn('curso', 'Curso', 'left', '10%');
        $col_datadevolucao = new TDataGridColumn('datadevolucao', 'Data Devolução', 'left', '10%');
        ...
        $col_datadevolucao->setAction(new TAction([$this, 'onReload']), ['order' => 'datadevolucao']);
        ...
        $this->datagrid->addColumn($col_datadevolucao);
        
        //$action1 = new TDataGridAction(['DevolvidosForm', 'onEdit'],   ['key' => '{id}'] );
    ...
    function onSearch()
    {
        ...
        if (isset($data->name) or (isset($data->matricula)))
        {
            if(isset($data->name)) {
                ...
                TSession::setValue('DevolvidosList_filter1', $filter1);
            } 
            if(isset($data->matricula)) {
                // creates a filter using what the user has typed
                $filter2 = new TFilter('matricula_id', 'like', "%{$data->matricula}%");
                // stores the filter in the session
                TSession::setValue('DevolvidosList_filter2', $filter2);
            } 
            
            TSession::setValue('Nome_Devolvidos_Filter',   $data->name);
    ...
    function onReload($param = NULL)
    {
        try
        {
            ...
            $filternaodevolvido = new TFilter('datadevolucao', 'IS NOT', NULL);
            $criteria->add($filternaodevolvido);
            
            if (TSession::getValue('DevolvidosList_filter1'))
            {
                // add the filter stored in the session to the criteria
                $criteria->add(TSession::getValue('DevolvidosList_filter1'));
            }

            if (TSession::getValue('DevolvidosList_filter2'))
            {
                // add the filter stored in the session to the criteria
                $criteria->add(TSession::getValue('DevolvidosList_filter2'));
            }
            ...
            if ($objects)
            {
                ...
                foreach ($objects as $object)
                {
                    ...
                    $object->datadevolucao = TDate::date2br($object->datadevolucao);
